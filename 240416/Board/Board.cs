...
public class Board : MonoBehaviour
{
    ...
    Cell CurrentCell
    {
        ...
        set
        {
            ...
            {
                currentCell?.RestoreCovers();   // 이전 currentCell이 눌려놓았떤 것을 모두 원래대로 복구
                currentCell = value;
    ...
    /// <summary>
    /// 보드에 마우스 왼쪽 버튼이 눌렸을 때 실행될 델리게이트
    /// </summary>
    ...
    public void Initialize(int newWidth, int newHeight, int newMineCount)
    {
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                cell.onFlagUse += gameManager.DecreaseFlagCount;    // 셀에 깃발이 설치되었을 때 실행될 함수 연결
                cell.onFlagReturn += gameManager.IncreaseFlagCount; // 셀의 깃발이 제거되었을 때 실행될 함수 연결
                ...
                cell.onCellOpen += () =>                    // 셀이 열릴 때마다 클리어 체크용 람다 함수 실행
                {
                    ...
                    if (closeCellCount == mineCount)        // 닫힌 셀의 개수가 지뢰 개수와 같다 == 게임이 클리어 되었다.
                    {
    ...
    /// <summary>
    /// 게임 오버가 되면 보드가 처리할 일을 기록해놓은 함수
    /// </summary>
    private void OnGameOver()
    {
        //Debug.Log("보드 : 게임 오버 신호를 받음");
    ...
    /// <summary>
    /// 게임 클리어가 되면 보드가 처리할 일을 기록해놓은 함수
    /// </summary>
    private void OnGameClear()
    {
        notFoundMineCount = 0;              // 클리어면 못찾은 지뢰가 없다.

        if (gameManager.FlagCount != 0)     // 아직 설치 안한 깃발이 있으면
        {
            foreach (Cell cell in cells)
            {
    ...
    /// <returns>보드 안이면 true, 밖이면 false, 셀이 초기화되지 않아도 false</returns>
    ...
    {
        if (Mouse.current.leftButton.isPressed) // 마우스 왼쪽 버튼이 눌려진 상태 확인
        {
    ...
    /// <param name="result">셔플된 경과</param>
